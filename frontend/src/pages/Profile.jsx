...
function Profile() {
    ...
    const [userUpdateDetails, setUserUpdateDetails] = useState({
        ...
        avatar: userProfile?.avatar ?  userProfile?.avatar.replace(/^http:/, "https:") : userProfile?.avatar || "",
    ...
    useEffect(() => {
        const userDetails = async () => {
            ...
            setUserUpdateDetails({ fullName: userInfo?.data?.data?.fullName, email: userInfo?.data?.data?.email, avatar: userInfo?.data?.data?.avatar ? userInfo?.data?.data?.avatar.replace(/^http:/, "https:") : userInfo?.data?.data?.avatar, coverImage: userInfo?.data?.data.coverImage })
        }
        userDetails()
    }, [])
